...
def cli_EDIWN_kinetics_plot(df, output_folder, exp_name):
    ...
    if n_number == 1:
        ...
        ax.set_title(f'{exp_name} Kinetics Scan (N={n_number}, Band=Standard Deviation)', wrap=True) # adds a title to the graph with the correct n number
...
def EDWIN_kinetics_analysis(data_file, **kwargs):
    ...
    df2['Condition'] = df2['Group'].replace(res) # creates a new column with the name of each condition for easier reference
