...
def interpret_plate_kinetics(data_file, labels):
    ...
    df2['Converted_Time'] = (((df2['Extracted_Minute'] * 60) + df2['Extracted_Second']) / 60).round(
        2)  # Converts time to a minute decimal
    ...
    df2.to_csv(os.path.join(base_folder, '%s_Formatted_Data.csv' % exp_name), encoding='utf-8',
               index=False)  # Exports the formatted datafram as a CSV file

    summary = df2.groupby(["Group", "Converted_Time"])["Signal"].describe()  # Produces data summary for group/timepoint
    summary.to_csv(os.path.join(base_folder, '%s_Data_Summary.csv' % exp_name),
                   encoding='utf-8')  # Exports a data summary to a csv file

    max_time = df2['Converted_Time'].max()  # To determine the max time for the x axis limit

    sns.color_palette("colorblind")

    ax = sns.lineplot(data=df2, x='Converted_Time', y='Signal', hue='Group', ci="sd",
    ...
    plt.savefig(os.path.join(base_folder, '%s_Plate_Kinetics' % exp_name), bbox_inches='tight',
