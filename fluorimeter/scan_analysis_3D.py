...
def cli_plot_heatmap(
    ...
    hmap = sns.heatmap(
        heatmap_data,
        xticklabels=["%.2f" % e for e in excitation_wavelengths],
        # changes the dp of the excitation wavelength
        yticklabels=emission_wavelengths,
        vmin=0,
        vmax=c_axis,
        cmap="viridis",
    ...
    hmap.set_xticklabels(
        ["%.3f" % e for e in excitation_wavelengths]
    )  # converting floats to 3DP strings
    hmap.set_yticklabels(
        ["%.3f" % e for e in emission_wavelengths]
    )  # converting floats to 3DP strings
    ...
    plt.savefig(
        os.path.join(base_folder, "%s_3DScan%s.png" % (exp_name, sample_info)), dpi=300
    )
...
def interpret_3d_scan(
    data_file,
    plot_separately,
    preformatted_data=False,
    save_formatted_data=False,
    **kwargs
):
    ...
    if not preformatted_data:
        ...
        raw_data = raw_data.loc[
            :, ~(raw_data == "").any()
        ]  # if scan stopped before fully complete, removes any unfinished columns

        raw_data = raw_data.astype(float)

    else:
        raw_data = pd.read_csv(
            os.path.join(base_folder, "%s_Formatted_Data.csv" % exp_name), header=[0, 1]
        )
    ...
    if (
        "_EX_" in raw_data.columns.to_list()[0]
    ):  # defines which type of wavelength is in the header/columns
        wavelengths["emission"] = raw_data.iloc[
            :, 0
        ].to_list()  # column wavelengths repeated throughout file
        ...
        raise RuntimeError(
            "Something is wrong in the data header - add either emission or excitation data only."
        )
    ...
    sample_names = []
    for i in range(
        0, len(header_wavelengths), 2
    ):  # run through the header data, the wavelength is in every second cell
    ...
    for index, wav in enumerate(
        header_wavelengths_reps
    ):  # extracting positions of any duplicates (for averaging down the line)
        header_wavelength_positions[wav].append(index)

    wavelengths[header_type] = list(header_wavelength_positions.keys())

    replicates = len(
        list(header_wavelength_positions.values())[0]
    )  # extracts number of replicates used from data

    if not plot_separately:
        print("Averaging samples")
        heatmap_data = np.zeros(
            (len(wavelengths["emission"]), (len(wavelengths["excitation"])))
        )  # emission on y-axis, excitation on x-axis
        for i, ex in tqdm(
            enumerate(wavelengths[column_type]),
            total=len(wavelengths[column_type]),
            ...
            for j, (wav, wav_indices) in enumerate(header_wavelength_positions.items()):
                ...
                heatmap_data[heatmap_ordering] = np.mean(
                    replicate_data
                )  # averaging data according to number of replicates
        ...
        for s_index, sample in enumerate(sample_names):
            ...
            for i, ex in tqdm(
                ...
                for j, (wav, wav_indices) in enumerate(
                    header_wavelength_positions.items()
                ):
                    if header_type == "emission":
                        heatmap_ordering = (j, i)
                    else:
                    ...
                    if s_index >= len(
                        wav_indices
                    ):  # sometimes not all wavelengths produced for every sample
