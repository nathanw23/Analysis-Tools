...
def cli_multiplex_plot(df, output_folder, exp_name):
    ...
    grid.map(
        sns.lineplot, "Time (min)", "Intensity (A.U.)", ci="sd", palette="colorblind"
    )
...
def fluorimeter_multiplex_analysis(data_file):
    ...
    df_combined[["Group", "Wavelengths"]] = df_combined["Group"].str.split(
        "_", 1, expand=True
    )  # Splits the wavelengths from the sample names
    ...
    res = {
        wavelengths[i]: fluorophores[i] for i in range(len(wavelengths))
    }  # Creates a dictionary

    df_combined["Fluorophore"] = df_combined["Wavelengths"].replace(
        res
    )  # Creates a new column with the name of the fluorophores for easier reference
